...
const ctx = window.STACKING_CONTRACT_V2.split(".")[0]
const ctx_name = window.STACKING_CONTRACT_V2.split(".")[1]
...
export default {
	...
	currentStakingIds: async (args = {address: null}, UserState, cb = null, ecb = null ) => {
		try {

			let result = await callReadOnlyFunction({
			  contractAddress: ctx,
			  contractName: ctx_name,
			  functionName: 'current-staking-ids',
			  network: globals.NETWORK,
			  functionArgs: [standardPrincipalCV(args.address)],
			  senderAddress: UserState.userData.profile.stxAddress[globals.SELECTED_NETWORK_CALLER]
			})
	...
	claim: async (args = {stacking_id: null, nft_id: null, collection: null, token_name: null}, UserState, doContractCall, cb = null, ecb = null ) => {
		try {	
			...
			//const assetAddress = window.TOKEN_CONTRACT.split('.')[0];
			//const assetContractName = window.TOKEN_CONTRACT.split('.')[1];
			//const fungibleAssetInfo = createAssetInfo(assetAddress, assetContractName, window.TOKEN_NAME);
			//console.log('ASSET', postConditionAddress, postConditionCode, postConditionAmount, fungibleAssetInfo)
//
			//const standardFungiblePostCondition = makeContractFungiblePostCondition(
			  //window.STACKING_CONTRACT_V2.split('.')[0],
			  //window.STACKING_CONTRACT_V2.split('.')[1],
			  //postConditionCode,
			  //postConditionAmount,
			  //fungibleAssetInfo
			//);
//
			//console.log('pc', standardFungiblePostCondition)
//
//
	...
	unstake: async (args = {stacking_id: null, nft_id: null, collection: null, token_name: null}, UserState, doContractCall, cb = null, ecb = null ) => {
		try {	
			console.log('unstake', args)
			let post_conditions = []
