...
function BonusAddresses (props) {
	...
	return <div>
		...
		<Col lg={6} md={6} sm={12}>
	  	...
	  	<Col sm={12}>
	  		<h3 className="subtitle no-border" style={{marginTop: 40}}>Set bonus percentage</h3>
	  		<FormGroup floating style={{marginTop: 24}}>
				<Input value={percentage} id="add_percentage" onChange={(e)=>setPercentage(e.target.value)} />
				<Label for="add_percentage">
			        Bonus percentage (percentage is /100 - to set 1% set 100, to set 0.01% set 1)
		      	</Label>
			</FormGroup>
			<Button color="primary" block style={{color: '#fff'}} className="mt-3" size="lg" onClick={async () => {
				if(setting_percentage) return;

				if(parseInt(percentage) < 0 || parseInt(percentage) > (100*100)) {
					alert('INVALID VALUE (0-10000)');
					return;
				}
				
				setSettingPercentage(true)
				
				token.setBonusPercentage(
					{ percentage: parseInt(percentage) },
					UserState,
					doContractCall,
					(res)=>{
						setSettingPercentage(false)
						setPercentage(null)
					},
					(err) => {
						setSettingPercentage(false)
				...
				{setting_percentage ? <Spinner size="sm" /> : <b>CONFIRM AND REMOVE</b>}
			</Button>
	  	</Col>
	</Row> 
	</div>
